...
class BookKafkaResourceIT {
    ...
    void setup() {
        ...
        BookKafkaResource kafkaResource = new BookKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/book-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/book-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
